...
public class  Catalog {
  ...
  public static Map<String, Collection<Television>> findByBrands(String... brands) {
    ...
    Map<String, Collection<Television>> tvsMap = new HashMap<>();

    for (String brand : brands) {
      if (catalog.contains(brand)) {
        tvsMap.put(brand, new ArrayList<>());
      }
    }

    for (Television item : catalog) {
      String curKey = item.getBrand();
      if (curKey.equals(brands)) {
        Collection<Television> currentCollection = tvsMap.get(curKey);
